...
export function evaluateClaim(evidence: ClaimEvidence): ClaimEvaluation {
  if (!evidence.identityMatch) return { decision: 'rejected', score: 0, reasons: ['身份信息不一致'] }
  let score = 55
  const reasons = ['身份摘要一致']
  ...
  const decision: ReviewDecision = score >= 80 ? 'approved' : score < 45 ? 'rejected' : 'review'
