...
`identityBindings` 使用学号 HMAC 作为文档 `_id`，不需要额外索引。它用于保证同一个学号不能被多个微信账号重复绑定。首次保存身份后，姓名和学号会锁定，状态为 `pending`；管理员完成线下校园证件或学校系统核验后，才能把状态改为 `verified`。

如果云端已经有旧版 `users` 数据，用户下一次登录时会自动迁移为 `pending`，不会被自动视为已核验。管理员必须重新核验。
...
3. 管理员重新打开小程序，从「MY → 管理员控制台」进入审核队列。
4. 必须先查看线下校园证件或学校系统，再把证件上的完整姓名和 10 位学号录入管理员页面。服务端会与原绑定数据进行 HMAC 比对，仅在一致时批准；录入的明文不会保存。掩码姓名和掩码学号本身不能作为身份依据。
5. 退回身份申请会释放该学号绑定，用户可以修正后重新提交。
...
3. 管理员完成线下身份核验并批准；未批准前，SEARCH 和失卡登记应被服务端拒绝。
4. 用已核验测试学号登记一张失卡。
5. 用另一个微信测试用户发布相同学号的拾卡记录。
6. 原失主进入「MY → 消息提醒」，应看到“发现相似校园卡”。
7. SEARCH 页面只显示隐藏后的姓名、学号和拾取地点大类，不能显示具体存放地点。
8. 失主提交认领申请，管理员批准后，「MY → 我的认领」才可以显示官方交接地点。
9. 管理员现场核验证件并点击“确认归还”，卡片、认领申请和失卡登记都应进入已归还状态。
